...
if [[ $1 = "install" ]]; then
  ...
  docker run -d --net hadoopnet --ip 172.20.1.4 --hostname psqlhms --add-host nodemaster:172.20.1.1 --add-host node2:172.20.1.2 --add-host node3:172.20.1.3 --add-host psqlhms:172.20.1.4 --add-host edge:172.20.1.5 --add-host hbase:172.20.1.9 --name psqlhms -e POSTGRES_PASSWORD=hive -it sciencepal/hadoop_cluster:postgresql-hms
  sleep 5
  
  # 3 nodes
  echo ">> Starting master and worker nodes ..."
  docker run -d --net hadoopnet --ip 172.20.1.1 -p 4040:4040 -p 8088:8088 -p 50070:50070 -p 8080:8080 -p 18080:18080 --hostname nodemaster --add-host node2:172.20.1.2 --add-host node3:172.20.1.3 --add-host psqlhms:172.20.1.4 --add-host edge:172.20.1.5 --add-host hbase:172.20.1.9 --name nodemaster -it sciencepal/hadoop_cluster:hive
  ...
  docker run -d --net hadoopnet --ip 172.20.1.9 -p 16010:16010 -p 2182:2181 --hostname hbase --add-host nodemaster:172.20.1.1 --add-host node2:172.20.1.2 --add-host node3:172.20.1.3 --add-host psqlhms:172.20.1.4 --add-host edge:172.20.1.5 --add-host hbase:172.20.1.9 --name hbase -it sciencepal/hadoop_cluster:hbase 
...
if [[ $1 = "uninstall" ]]; then
  ...
  docker rmi sciencepal/hadoop_cluster:hadoop sciencepal/hadoop_cluster:spark sciencepal/hadoop_cluster:hive sciencepal/hadoop_cluster:postgresql-hms sciencepal/hadoop_cluster:hue sciencepal/hadoop_cluster:edge sciencepal/hadoop_cluster:hbase -f
  exit
fi
